...
int pow(int base, int exponent, long long int modulus)
{
    int result = 1;
    base = base % modulus;
    while (exponent > 0)
...
int order(long long int p, int b)
{
    if (gcd(p, b) != 1)
    {
...
int STonelli(int n, long long int p)
{
...
int main()
{
    ...
    long long int p = 13710221545914561761;
 	long long int q=11066328760152681859;
    int x = STonelli(n, p);
 
    if (x == -1)
        printf("Modular square root is not exist\n");
    else
        printf("Modular square root of %d and %lld is %d\n",
                n, p, x);
	int y=STonelli(n, q);
if(x==-1)
printf("doesnt exist");
